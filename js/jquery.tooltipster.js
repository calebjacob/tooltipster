	...
	Plugin.prototype = {
			...
			if (self.options.functionBefore.call(self, self.$el[0]) !== false) {
				...
				if (self.enabled && self.Content !== null) {
					...
					// if the tooltip isn't already open, open that sucker up!
						...
						// only add the animation class if the user has a browser that supports
						// animations
						if (supportsTransitions()) {
							self.$tooltip
		...
		reposition: function() {
			...
			if ($('body').find(self.$tooltip).length !== 0) {
				...
				var result = this.displayPlugin.reposition({
					// the geometry helper
				...
				self.elProxyPosition = self._geometry(self.$elProxy);
...
;(function($) {
	...
	plugin.prototype = {
		...
		reposition: function(helper) {
			...
			// if everything failed, fall back on the preferred position
			if (!result) {
