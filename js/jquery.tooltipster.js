...
;(function($) {
	...
	var defaults = {
		...
		instancesLatest = [],
		// list of static methods, usable by calling $.fn.tooltipster('methodName')
		staticMethods = {
			/**
			...
			/**
			 * Allows plugins to create new static methods
			 * 
			 * @param {string} name
			 * @param {function} fn
			 */
			staticMethodAdd: function(name, fn){
	...
	$.fn.tooltipster = function() {
		...
		if (this.length === 0) {
			...
			if (typeof args[0] === 'string') {
				...
				if (staticMethods[args[0]]) {
					ret = staticMethods[args[0]].apply(this, args.slice(1));
				}
				else {
				...
				// $(sel).tooltipster('methodName') calls with $(sel) matching
				// nothing will return an empty collection of objects because
				// chaining is sometimes expected when working on empty
				// collections. If the user actually called a static method
				// which does not exist... too bad.
				else return this;
			}
